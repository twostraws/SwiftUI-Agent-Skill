...
- When using `if #available(...)` checks, verify them against the project's minimum deployment target. If the minimum target matches or exceeds the condition (e.g., iOS 26+ project checking `if #available(iOS 26, *)`), flag the unnecessary check to the user for manual removal rather than auto-deleting—developers may have intentionally kept older checks for documentation purposes or to support conditional compilation across targets.
